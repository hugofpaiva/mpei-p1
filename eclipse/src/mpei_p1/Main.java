...
public class Main {
	...
	private static void teste() throws InterruptedException {
		...
		MinHash lol = new MinHash(jogos.get(0).getReviews(), 10); //10 pq temos comentários grandes, senão era 5
		lol.getSimilar();
		lol.printSimilar();
		...
		for(Game jogo:jogos) {
			for(Language lang:jogo.getLanguages()) {
				if(!languages.contains(lang.getName())) {
					...
					CBloom l_audioBloom = new CBloom(jogos.size(), 0.1, maxCG); // bloom 
